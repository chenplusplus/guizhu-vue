<!-- src/views/process/xs-list.vue 镶石单据列表 -->
<template>
  <div class="page-container" v-loading="loading">
    <div class="page-header">
      <div class="header-left"><h2>💎 镶石单据列表</h2><el-tag type="info">共 {{ total }} 条</el-tag></div>
      <div class="header-right">
        <el-button type="primary" @click="$router.push({ path: '/process/xs-input', query: { master: searchForm.masterName || undefined } })"><el-icon><Plus /></el-icon> 录入</el-button>
    ...
    <div class="table-section">
      <el-table :data="tableData" border stripe v-loading="loading">
        ...
        <el-table-column prop="sendWeightG" label="发出(g)" width="90" align="right"><template #default="{ row }">{{ row.sendWeightG?.toFixed(2) }}</template></el-table-column>
        <el-table-column prop="recycleWeightG" label="回收(g)" width="90" align="right"><template #default="{ row }">{{ row.recycleWeightG?.toFixed(2) }}</template></el-table-column>
        <el-table-column label="钻石信息" width="180" align="center">
          <template #default="{ row }">
            <span v-if="row.extraFields" class="diamond-info">
              微{{ row.extraFields.microCarat || 0 }}ct/{{ row.extraFields.microCount || 0 }}粒
              手{{ row.extraFields.handCarat || 0 }}ct/{{ row.extraFields.handCount || 0 }}粒
            </span>
            <span v-else>-</span>
          </template>
        </el-table-column>
        <el-table-column label="报废(g)" width="80" align="right">
          <template #default="{ row }"><span class="scrap-text">{{ row.extraFields?.scrapG || 0 }}</span></template>
        </el-table-column>
        <el-table-column prop="laborFee" label="加工费(元)" width="100" align="right"><template #default="{ row }">{{ (row.laborFee || 0).toFixed(2) }}</template></el-table-column>
        ...
        <el-table-column label="操作" width="220" fixed="right" align="center">
          <template #default="{ row }">
            <el-button size="small" type="primary" link @click="$router.push(`/process/xs-detail/${row.id}`)">查看</el-button>
...
const dateRange = ref([]);
const searchForm = reactive({ keyword: '', masterName: '', materialId: null, status: null, startDate: '', endDate: '' });
...
const loadData = async () => {
  loading.value = true;
  try {
    const res = await getProcessList({ mainProcess: 'XS', keyword: searchForm.keyword || undefined, masterName: searchForm.masterName || undefined, materialId: searchForm.materialId || undefined, status: searchForm.status ?? undefined, startDate: searchForm.startDate || undefined, endDate: searchForm.endDate || undefined, page: pagination.current, pageSize: pagination.pageSize });
    tableData.value = res.data || [];
    total.value = res.total || 0;
  } catch { ElMessage.error('加载失败'); } finally { loading.value = false; }
};
const resetSearch = () => { Object.assign(searchForm, { keyword: '', materialId: null, status: null, startDate: '', endDate: '' }); dateRange.value = []; pagination.current = 1; loadData(); };
const openEdit = (row) => router.push(`/process/xs-input?id=${row.id}`);
...
const confirmAudit = async () => {
  ...
  try { await auditProcessReceipt({ id: auditRow.value.id, approved: auditForm.approved, auditNote: auditForm.auditNote }); ElMessage.success(auditForm.approved ? '审核通过' : '已驳回'); auditVisible.value = false; loadData(); }
  catch (e) { ElMessage.error(e.message || '操作失败'); } finally { auditLoading.value = false; }
};
const loadOptions = async () => { try { const res = await getProcessOptions(); options.materials = res.data?.materials || []; options.zmSubProcesses = res.data?.zmSubProcesses || []; options.pgSubProcesses = res.data?.pgSubProcesses || []; } catch {} };
...
.scrap-text { color: #F56C6C; }
.diamond-info { font-size: 12px; color: #606266; }
</style>
