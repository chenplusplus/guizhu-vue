      ...
      <div class="bill-sheet" v-if="!loading && billData">
        ...
        <div class="sheet-table-wrapper">
          ...
          >
            ...
            <el-table-column prop="goldMaterialFee" label="足金料金额" width="80" align="right" />

            <!-- ⭐ 折足金料（克重） -->
            <el-table-column
              v-if="showMaterialWeightColumns"
              prop="goldMaterialWeight"
              label="折足金料(g)"
              width="100"
              align="right"
            >
              <template #default="{ row }">
                {{ (row.goldMaterialWeight || 0).toFixed(3) }}
              </template>
            </el-table-column>
        ...
        <div class="sheet-footer">
          ...
          <div class="footer-row">
            ...
            <div class="footer-cell">
              ...
              <span class="f-value highlight">{{ (billData.currentMaterialWeight || 0).toFixed(3) }}g</span>
            </div>
            <div class="footer-cell">
          ...
          <!-- ⭐ 内部统计：折足金料总克重（仅当有折现金时显示） 
          <div class="footer-row" v-if="showMaterialWeightColumns">
            <div class="footer-cell" style="flex:4;">
              <span class="f-label" style="color:#909399;">折足金料（内部）：</span>
              <span class="f-value" style="color:#909399;">{{ (billData.internalMaterialWeight || 0).toFixed(3) }}g</span>
            </div>
          </div>-->
...
// ===== ⭐ 只要有任意一行是折现金，就显示折足金料 + 折现金两列 =====
const showMaterialWeightColumns = computed(() => {
  return (rawDetails.value || []).some(d => d.goldConvertCash);
});
...
const tableData = computed(() => {
  ...
  order.forEach((custName) => {
    ...
    items.forEach((d) => {
      ...
      sub.goldMaterialWeight += Number(d.goldMaterialWeight || 0);  // ⭐ 新增
    ...
    rows.push({
      ...
      goldMaterialWeight: round3(sub.goldMaterialWeight),  // ⭐ 新增
  ...
  rows.push({
    ...
    goldMaterialWeight: round3(grand.goldMaterialWeight),  // ⭐ 新增
